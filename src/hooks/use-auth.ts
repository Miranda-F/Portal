    ...
    try {
      await retryWithBackoff(async () => {
        ...
        if (response.ok) {
          ...
          // Se userData for null, significa que o usuário não está logado (comportamento novo para evitar 401)
          if (userData === null) {
          ...
          // Unauthorized é esperado para usuários não logados (mantendo compatibilidade)
          updateAuthState(null, setUser, setLastAuthCheck, true) // Limpa cache
        } else {
    ...
    try {
      ...
      if (errorType !== ErrorType.TIMEOUT && errorType !== ErrorType.NETWORK) {
        toast({
          ...
          description: "Erro ao comunicar com servidor, mas logout local foi realizado.",
          variant: "destructive"
        })
      }
    } finally {
