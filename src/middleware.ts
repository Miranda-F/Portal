...
export async function middleware(request: NextRequest) {
  ...
  const protectedPaths = ['/admin', '/usuario', '/rh', '/eventos', '/qualidade', '/treinamentos']
  ...
  if (protectedPaths.some(path => request.nextUrl.pathname.startsWith(path))) {
    ...
    // Verificar se é rota de admin, eventos, qualidade ou treinamentos e se o usuário tem permissão
    if ((request.nextUrl.pathname.startsWith('/admin') || 
         request.nextUrl.pathname.startsWith('/eventos') || 
         request.nextUrl.pathname.startsWith('/qualidade') || 
